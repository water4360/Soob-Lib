...
// 						let select = window.confirm(msg + "할까요?");
				...
				<%-- 테이블 시작 --%>
					<table class="table table-hover">
						<thead>
							<tr>
								<th scope="col"><input type="checkbox" name="checkAll" onclick="toggleCheckAll()">
								...
								<c:forEach var="book" items="${books}">
									<tr>
										<th scope="row"><input type="checkbox" id="check" name="check"
												onclick="selectBook('${book.manageNo}')" data-status="${book.status}">
										</th>
													...
													<c:when test="${empty loginMember}">
														<div class="btn-group">
															<button type="button" class="btn btn-success"
																...
																onclick="redirectToLogin()" ${book.status=='0'
																? 'disabled' : '' }>
										...
										<td>
											<div class="btn-group">
												<button type="button" class="btn btn-danger" data-toggle="modal"
													data-target="#deleteBook" aria-expanded="false" id="btn-delete"
													${book.status=='0' ? 'disabled' : '' }>
			...
			<%-- 도서대여 모달 --%>
				<div class="modal fade" id="rentBook" tabindex="-1" aria-labelledby="newBookModalLabel"
					aria-hidden="true">
						...
						<div class="modal-content input-form mx-auto">
							...
							<%-- 모달 내용 --%>
								<div class="modal-body">
									<form method="post" action="rentBook.do" name="rentBookForm" class="validation-form"
										novalidate>
										...
										<div id="rentMsg">
											<h5>대여기간은 3일이예요.</h5>
											<h5>대여한 도서는 <span style="color:#28A745">
				...
				<%-- 대여버튼 클릭시 도서 정보가져오기 <script>
					...
					<%-- 도서 신규등록 모달 --%>
						<div class="modal fade" id="addNewBook" tabindex="-1" aria-labelledby="newBookModalLabel"
							aria-hidden="true">
							<div class="modal-dialog">
								<div class="modal-content input-form mx-auto">
									<div class="modal-header">
									...
									<%-- 모달 내용 --%>
										<div class="modal-body">
											<form method="post" action="add-book-process.do" name="addBookForm"
												...
												<div id="rentStatus">
													<label for="status1"><input type="radio" id="status1" name="status"
															value="1" checked> 대출활성화</label> <label for="status0"><input
															type="radio" id="status0" name="status" value="0">
														대출비활성화</label>
												</div>

												<label for="isbn">ISBN</label> <input type="number" class="form-control"
													id="isbn" name="isbn" autocorrect="off" autocapitalize="none"
													placeholder="공백이나 -없이 10~13자리" value="" onchange="checkIsbn()"
													required>
												...
												<label for="title">도서명</label> <input type="text" class="form-control"
													id="title" name="title" autocorrect="off" autocapitalize="none"
													placeholder="시선으로부터" value="" required>
												<div class="invalid-feedback">도서명을 입력해주세요</div>
												<div id="valid-feedback" style="color: green;"></div>

												<label for="author">저자</label> <input type="text" class="form-control"
													id="author" name="author" autocorrect="off" autocapitalize="none"
													placeholder="정세랑" value="" required>
												...
												<label for="publisher">출판사</label> <input type="text"
													...
													autocorrect="off" autocapitalize="none" placeholder="문학동네" value=""
													required>
